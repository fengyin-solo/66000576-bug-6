...
export const useImagingStore = defineStore('imaging', () => {
    ...
    // 应用某部位最后使用（或默认）的窗方案
    function restoreWindowForPart(part) {
    ...
    function restoreSession() {
        let part = '';
        try {
            part = localStorage.getItem(LAST_PART_KEY) || '';
        }
        catch { /* ignore */ }
        if (!part)
            return false;
    ...
    function applyWindow(w, l) { setManualWindow(w, l); }
