...
export function defaultPresetForPart(part) {
 ...
 */
export function windowIntensity(value, width, level) {
    if (width <= 0)
        return value >= level ? 1 : 0;
    const lower = level - width / 2;
    const t = (value - lower) / width;
    return Math.max(0, Math.min(1, t));
}
/** 当前 WW/WL 是否与某个预设方案一致（用于面板高亮） */
export function matchPreset(p, width, level) {
